...
class FipsEnabledNginx < Formula
  ...
  bottle do
    ...
    revision 1
    sha1 "3dd3ecf42118a2612efc9bd62d6b7390d71d324f" => :mavericks
  end
  ...
  depends_on 'fips-enabled-openssl'
  ...
  def install
    ...
    pcre = Formula["pcre"]
    openssl = Formula["fips-enabled-openssl"]
    ...
    # for such cases
    if (HOMEBREW_CELLAR/'fips-enabled-nginx').subdirs.any?{|d| (d/:sbin).directory? }
      sbin.mkpath
      sbin.cd do
